...
const DashboardCollection = () => {
    ...
    return (
        ...
        <div className="">
            <div style={{ height: parseInt(heights) - parseInt(dashTopHeight) - 20 }} className="pt-3">
                <div className="flex flex-col grow h-full">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 justify-between gap-4 mt-5 pb-5 grow">
                        <div className="flex flex-col  border grow">
                            <div className="flex border-b items-center w-full bg-[#F3FBFB] p-4"><h1 className="text-[18px]">Collection by Terminals</h1></div>
                            <div className="flex flex-col px-4 gap-2 my-4">
                                {datas.map((data, index) => (
                                    <div key={index} className="flex w-full border-b py-2 justify-between">
                                        <div className="flex items-center gap-2">
                                            <img src={data.image} className="h-10 w-10" alt="" />
                                            <div className="ml-4">
                                                <div className="font-thin text-gray-900">{data.transactionName}</div>

                                            </div>
                                        </div>
                                        <div className="ml-4">
                                            <div className=" text-gray-900 flex justify-end">CFA {data.amount}</div>
                                        </div>
                                    </div>
                                ))}
                            </div>
                            ...
                            <div className="flex flex-col  border grow">
                                ...
                                <div className="flex flex-col px-4 gap-2 mt-4 divide-y">
                                    {Incomes.map((data, index) => (
                                        <div key={index} className="flex justify-between py-2 first:pt-0 items-center">
                                            <div className="flex items-center gap-2">
                                            ...
                                            <div className=" text-gray-900 text-lg">{data.amount}</div>
                                        </div>
                                    ))}
                                </div>
                            </div>
