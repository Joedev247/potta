'use client';
import DateRangePickerComponent from '@potta/components/daterangePicker';
...
const FilterComponent: FC<FilterComponentProps> = ({
  ...
  return (
    <div className="flex grow items-center gap-2">
      <div
        className={`flex items-center gap-x-3 border rounded-sm px-4 w-full md:w-2/4 ${
          includeSearch ? '' : 'hidden'
        }`}
      >
      ...
      <div
        title="Sort"
        className={`flex border rounded-sm cursor-pointer py-1.5 px-2 ${
          includeSort ? '' : 'hidden'
        }`}
        onClick={() => handleSort()}
      >
