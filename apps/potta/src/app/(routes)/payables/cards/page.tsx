...
const Card: FC<CardProps> = () => {
  ...
  const filteredCards = cards.filter((card) => {
    if (allCards) {
      return (
        card.cardNumber &&
        card.cardHolder.toLowerCase().includes(searchCardHolder.toLowerCase())
      );
      ...
      >
        <div className="flex justify-between">
          <div className="flex flex-col gap-2">
            ...
            <h1 className="text-gray-400 text-sm">
              Today balance from all accounts{' '}
        ...
        <div className="flex justify-between mt-5">
          <div className="flex  space-x-3 w-[50%]">
            ...
            <div
              ...
              className="flex h-12 mt-4 border   cursor-pointer py-1.5 px-2"
            >
              <i
                className={`${
                  dataIsSorted ? 'ri-sort-asc' : 'ri-sort-desc'
                } text-2xl`}
              ></i>
            </div>
            <div className="mt-4">
              <CustomPopover>
                <div className="flex gap-4 bg-gray-100 mt-7  px-3 py-1">
                  <div
                    ...
                    className={`flex text-center  py-1.5 cursor-pointer px-4 text-xs sm:text-base ${
                      allCards ? 'bg-black text-white' : 'text-black pr-4'
                    }`}
                  >
                    All
                  </div>
                  <div
                    ...
                    className={`flex text-center  py-1.5 cursor-pointer px-4 text-xs sm:text-base ${
                      physicalCards ? 'bg-black text-white' : 'text-black px-4'
                    }`}
                  >
                    Physical
                  </div>
                  <div
                    ...
                    className={`flex text-center  py-1.5 cursor-pointer px-4 text-xs sm:text-base ${
                      virtualCards ? 'bg-black text-white' : 'text-black px-4'
                    }`}
                  >
                    Virtual
                  </div>
                  <div
                    ...
                    className={`flex text-center  py-1.5 cursor-pointer px-4 text-xs sm:text-base ${
                      debitCards ? 'bg-black text-white' : 'text-black px-4'
                    }`}
                  >
                    Debit
                  </div>
                  <div
                    ...
                    className={`flex text-center  py-1.5 cursor-pointer px-4 text-xs sm:text-base ${
                      creditCards ? 'bg-black text-white' : 'text-black px-4'
                    }`}
                  >
                    Credit
                  </div>
          ...
          <div className="flex items-center gap-3">
            <div className="sm:flex divide-x border  hidden h-12 cursor-pointer">
              <div
                ...
                className={`flex px-4   items-center justify-evenly ${
                  listView ? 'bg-gray-100' : 'bg-white'
                }`}
              >
            ...
            <div className="flex flex-col mt-5">
              ...
              <div className="flex flex-col gap-3 mt-3">
                {filteredCards2.map((card, index) => (
                  <Link
                    key={'filteredCards2' + index}
                    href={`/sidebar/card/detail/${card.id}`}
                  >
                    <div
                      key={index}
                      className="flex justify-between border  px-7 py-3"
                    >
                      ...
                      <div className="flex flex-col gap-1">
                        <span
                          className={`py-1.5 w-24 text-center  font-medium text-md ${
                            card.status == 'Active'
        ...
        <div className={`block ${gridView ? '' : 'block sm:hidden'} pb-5`}>
          <div className="grid grid-cols-1 gap-6 mt-5 sm:grid-cols-2 xl:grid-cols-3">
            {filteredCards2.map((card, index) => (
              <Link
                key={'filteredCards21' + index}
                href={`/sidebar/card/detail/${card.id}`}
              >
                <div
                  key={index}
                  className="flex flex-col justify-between border  px-4 py-6"
                >
                  ...
                  <div className="flex justify-between items-center mt-8">
                    <div className="flex flex-col gap-1">
                      ...
                      <h1 className="text-gray-400 text-sm">
                        Blocked amount {card.cardCurrency}
                        {card.blockedAmount}
                      </h1>
                    </div>
                    <div className="">
