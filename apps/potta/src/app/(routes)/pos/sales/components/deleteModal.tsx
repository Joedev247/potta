...
import { useDeleteSalesReceipt } from "../hooks/useDeleteReceipt";
...
const DeleteModal: React.FC<DeleteProps> = ({ recieptID,  open: controlledOpen,
  setOpen: setControlledOpen }) => {
  const mutation = useDeleteSalesReceipt();
// Local state as fallback if no controlled state is provided
  ...
  const handleDelete = () => {
    mutation.mutate(recieptID, {
      onSuccess: () => {
        setIsOpen(false)
        toast.success("Sales Reciept deleted successfully!");
      }, // Renamed to avoid naming conflict
      onError: (error) => {
        console.log(error);
        setIsOpen(false)
        toast.error("Failed to delete Reciept, please try again later.");
      },
    });
  };
