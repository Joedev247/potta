...
├── page.tsx                          # Main page (token validation, data fetching)
├── components/
│   ├── KYCVerificationForm.tsx       # Main form UI
│   └── DocumentUploader.tsx          # Drag & drop upload component
├── hooks/
│   ├── useKYCVerification.ts         # Fetch KYC data
│   ├── useKYCSubmission.ts           # Submit documents
│   ├── useKYCDocumentUpload.ts       # Upload individual doc
│   ├── useKYCStatus.ts               # Check status
│   └── index.ts                      # Exports
├── services/
│   └── kycService.ts                 # API service layer
├── types/
