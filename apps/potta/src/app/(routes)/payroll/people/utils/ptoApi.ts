...
export const ptoApi = {
  ...
  accrueLeave: async (id: string, amount: number) => {
    const result = await axios.put(`/api/paid-time-off/accrue/${id}`, { amount });
    return result.data;
  },
  ...
  approveLeave: async (requestId: string) => {
    const result = await axios.put(`/api/paid-time-off/approve-leave/${requestId}`, {});
    return result.data;
  },

  // Reject a PTO leave request
  rejectLeave: async (requestId: string, reason: string) => {
    const result = await axios.put(`/api/paid-time-off/reject-leave/${requestId}`, { reason });
    return result.data;
  },
